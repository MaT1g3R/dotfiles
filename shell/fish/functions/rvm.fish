function rvm --description='Ruby enVironment Manager'
  ...
  bash -c 'PATH=$GEM_HOME/bin:$PATH;RVMA=$(which rvm 2>/dev/null);RVMB=$(whereis rvm | sed "s/rvm://");source $(if test $RVMA;then echo $RVMA | sed "s/\/bin\//\/scripts\//";elif test $RVMB; then echo $RVMB | sed "s/rvm/rvm\/scripts\/rvm/"; else echo ~/.rvm/scripts/rvm; fi); rvm "$@"; status=$?; env > "$0"; exit $status' $env_file $argv

  # apply rvm_* and *PATH variables from the captured environment
  and eval (/usr/bin/grep -E '^rvm|^PATH|^GEM_PATH|^GEM_HOME' $env_file | /usr/bin/grep -v '_clr=' | sed '/^[^=]*PATH/s/:/" "/g; s/^/set -xg /; s/=/ "/; s/$/" ;/; s/(//; s/)//')
